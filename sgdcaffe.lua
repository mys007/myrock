...
function optim.sgdcaffe(opfunc, x, config, state)
   -- (0) get/update state
   ...
   if mom ~= 0 then
    ...
    ----- NOTE: the effective learning rate is then ~ clr * 1/(1-momentum)  [assuming constant dfdx, rewrite the update recursion to clr*dfdx*sum_i{0,inf}(momentum^i)]
   ...
   return x,{fx},{-clr, lrs and state.deltaParameters or dfdx}
end
